##' @title combine 2 matchings
##' @description if a set of individuals have been matched to two different controls, this
...
##' @param trv value in tr indicating treatment, if different in x and y,
##'     supply a vector
...
##'     all individuals
##' @export
match_combine <- function(x, y, id, cl, tr, trv = 1, progress = FALSE){
    P <- progress
    if(P) cat("Starting match_combine\n")
    id1 <- id[1]
    ...
    tr.v1 <- trv[1]
    tr.v2 <- if(length(trv)>1) trv[2] else trv[1]
    ## -------------------------------------
    df1 <- x[, c(id1, cl1, tr1)]
    ...
    Z <- rbind(X1, subset(Y1, Y1$group != "case"))
...
if(FALSE){
    ...
    (R <- match_combine(x = df1, y = df2, id = c("id1", "id2"),
