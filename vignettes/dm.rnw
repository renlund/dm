...
with a fairly complex recoding to also illustrate the use the \verb|recode|
...
\clearpage
\section{\texttt{dmf} and related functions}
...
\subsection{Toy example continuation}
...
(\verb|grep| by \textbf{i}ndividual, (possibly) \textbf{c}onstrained in
\textbf{t}ime.)
...
creating an clinical data base:
\begin{itemize}
